...
def test_personality_presets_catalog() -> None:
    ...
    assert first.get("biography_preview")
    assert first.get("expertise_preview")
    ep = first.get("expertise_profile")
    assert isinstance(ep, dict)
    assert ep.get("core_expertise")
    assert ep.get("strong_familiarity")
    assert ep.get("weak_or_neutral")
